...
async def wait_boundary(session: LiveMatchSession) -> None:
    ...
    while session.turn_index and not ready(session):
        ...
        if (elapsed >= session.response_timeout and
                session.phase == "game_initialized" and
                session.recovery_epoch == epoch):
            session.phase = "paused_recovering"
            session._save("phase", session.phase)
            session.events.emit("paused", turn=session.turn_index,
                                phase=session.phase)
            paused = True
        if elapsed >= max(session.watchdog_timeout, session.response_timeout):
            abort(session, "stage4_boundary_expired", f"action-{session.turn_index - 1}")
            raise RuntimeError("Stage 4 boundary watchdog expired")
        await asyncio.sleep(0.02)
    if (paused and session.phase == "paused_recovering" and
            session.recovery_epoch == epoch):
