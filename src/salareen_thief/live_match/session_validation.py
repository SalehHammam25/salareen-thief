...
def validate_semantics(session: Any, tool: str, payload: dict[str, Any]):
    ...
    if tool == "security_commit_v1":
        if not session.security.peer_verified:
            return "SECURITY_REQUIRED", "bootstrap"
        return (
            None
            if payload["turn_index"] == session.turn_index
            else ("INVALID_TURN", "turn_index")
        )
    if tool == "security_nonce_audit_v1":
        return (
            None
            if session.phase in {"terminal", "shutdown"}
            else ("ILLEGAL_PHASE", "phase")
        )
    if tool == "initialize_game_v1":
        if session.security and not session.security.peer_verified:
            return "SECURITY_REQUIRED", "bootstrap"
        if session.phase not in {"configured", "game_initialized"}:
            return "ILLEGAL_PHASE", "phase"
        return (
            None
            if payload["starting_role"] == "thief"
            else ("INVALID_ROLE", "starting_role")
        )
    if tool == "resume_match_v1":
        received = session.journal.get_state(
            session.game_id, session.session_id, "last_received_turn"
        )
        pending = received is not None and payload["turn_index"] == int(received)
        if (payload["turn_index"] != session.turn_index and not pending) or payload[
            "phase"
        ] != session.phase:
            session._save("phase", "aborted")
    ...
    if session.phase in {"terminal", "shutdown"} and tool not in allowed:
        return "EPISODE_TERMINAL", "phase"
    if tool == "submit_action_v1":
        if session.security:
            try:
                session.security.accept_reveal(payload["correlation_id"], dict(payload))
            except (KeyError, ValueError):
                return "SECURITY_REJECTED", "reveal"
        issue = protocol.validate_action(payload, session.turn_index)
        return issue or (
            session.gameplay and session.gameplay.validate_payload(payload)
        )
