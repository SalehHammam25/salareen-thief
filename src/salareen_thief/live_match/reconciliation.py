...
def _event(session: LiveMatchSession, kind: str, correlation: str,
...
async def finish(url: str, session: LiveMatchSession, outcome: str,
                 scores: tuple[int, int]) -> None:
    winner = "cop" if outcome == "cop_capture" else "thief"
    loser = "thief" if winner == "cop" else "cop"
    messages = (("reconcile_terminal_v1", "terminal", {"outcome": outcome,
    ...
    event_names = {"reconcile_terminal_v1": "terminal_agreed",
