...
def action(**changes):
    value = {"protocol_version": "1.0-provisional", "correlation_id": "a-1",
             "sender_role": "cop", "game_id": "game", "session_id": "session",
             "game_number": 1, "turn_index": 1, "action_kind": "stay",
...
def test_expected_role_and_duplicate_mismatch(tmp_path):
    session = LiveMatchSession("thief", "game", "session", 1,
                               Journal(tmp_path / "thief.sqlite3"))
    session.turn_index = 1
...
def test_endpoint_policy_rejects_query():
    assert validate_endpoint("http://127.0.0.1:8802/mcp", mode="local",
                             host="127.0.0.1", permitted_port=8802)
    try:
        validate_endpoint("https://peer.example:443/mcp?token=x", mode="remote",
                          host="peer.example", permitted_port=443)
    except ValueError:
        pass
    else:
