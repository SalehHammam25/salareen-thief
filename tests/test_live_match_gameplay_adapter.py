...
def intent(**changes):
    value = {"protocol_version": "1.0-provisional", "correlation_id": "move-1",
             "sender_role": "cop", "game_id": "game", "session_id": "session",
             "game_number": 1, "turn_index": 1, "action_kind": "move",
             "direction": "S", "x": None, "y": None}
...
def test_barrier_replaces_movement_and_emits_no_scent(tmp_path):
    ...
    session = LiveMatchSession("thief", "game", "session", 1,
                               Journal(tmp_path / "thief.sqlite3"), gameplay)
    session.turn_index = 1
    result = session.handle("submit_action_v1", intent(
        action_kind="barrier", direction=None, x=0, y=1))
...
def test_acknowledgement_applies_prepared_local_action_once(tmp_path):
    ...
    session = LiveMatchSession("thief", "game", "session", 1,
                               Journal(tmp_path / "thief.sqlite3"), gameplay)
    local = intent(correlation_id="thief-0", sender_role="thief", turn_index=0,
                   action_kind="stay", direction="STAY")
    assert session.prepare_local(local)["accepted"]
    acknowledgement = {"protocol_version": "1.0-provisional",
        "correlation_id": "ack-0", "sender_role": "cop", "game_id": "game",
        "session_id": "session", "game_number": 1, "turn_index": 0,
        "action_correlation_id": "thief-0", "result": "applied",
        "result_code": "OK", "next_turn_index": 1, "next_role": "cop"}
